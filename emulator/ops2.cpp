	...
	u8 op2)
{
	...
	*/
	// Above replaced by below. -Wade Fife (WSF)
	SimControl.SimIO.PrintMessage("Opcode %02Xh encountered at address %04X:%04X.\n", (unsigned)op2, (unsigned)M.x86.R_CS, (unsigned)(M.x86.R_IP-2) & 0xFFFF);
	SimControl.SystemHalted(EXCEPTION_BAD_EXT_OPCODE);
	//
    HALT_SYS();
    END_OF_INSTR();
}
...
void x86emuOp2_set_byte(u8 op2)
{
    ...
    case 0:
        ...
        TRACE_AND_STEP();
        // Following line changed from "store_data_byte(destoffset, cond ? 0x01 : 0x00);" by Wade Fife, May 9, 2002
        store_data_byte(destoffset, (u8)(cond ? 0x01 : 0x00));
        break;
    case 1:
...
void x86emuOp2_bts_R(u8 X86EMU_UNUSED(op2))
{
    ...
    case 0:
        if (M.x86.mode & SYSMODE_PREFIX_DATA) {
			...
			mask = (u16)(0x1 << bit);
            CONDITIONAL_SET_FLAG(srcval & mask,F_CF);
            // Following line changed from "store_data_word(srcoffset+disp, srcval | mask);" by Wade Fife, May 9, 2002
...
void x86emuOp2_btX_I(u8 X86EMU_UNUSED(op2))
{
    ...
    default:
		...
		printk("%04x:%04x: %02X%02X ILLEGAL EXTENDED X86 OPCODE EXTENSION!\n",
                M.x86.R_CS, M.x86.R_IP-3, op2temp, (mod<<6)|(rh<<3)|rl);
		// Above line changed from "M.x86.R_CS, M.x86.R_IP-3,op2, (mod<<6)|(rh<<3)|rl);", by Wade Fife, May 9, 2002 (WSF)
		*/
		// Above replaced below. - Wade Fife (WSF)
		SimControl.SimIO.PrintMessage("%02X%02Xh encountered at address %04x:%04x\n.", op2temp, (mod<<6)|(rh<<3)|rl, M.x86.R_CS, M.x86.R_IP-3);
		SimControl.SystemHalted(EXCEPTION_BAD_EXT_OPCODE);
		//
        HALT_SYS();
    }
    ...
    case 0:
        if (M.x86.mode & SYSMODE_PREFIX_DATA) {
            ...
            CONDITIONAL_SET_FLAG(srcval & mask,F_CF);
            switch (rh) {
            // Following 3 cases added type cast to bitwise operators
			// For example, case 4 changed from "store_data_word(srcoffset, srcval | mask);" by Wade Fife, May 9, 2002
            case 4:
                store_data_word(srcoffset, (u16)(srcval | mask));
                break;
            case 5:
                store_data_word(srcoffset, (u16)(srcval & ~mask));
                break;
