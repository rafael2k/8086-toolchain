    ...
    u8 op1)
{
	...
	*/
	...
	SimControl.SimIO.PrintMessage("Opcode %02Xh encountered at address %04X:%04.4X.\n", (unsigned)op1, (unsigned)M.x86.R_CS, (unsigned)(M.x86.R_IP-1) & 0xFFFF);
	SimControl.SystemHalted(EXCEPTION_BAD_OPCODE);
	//
    HALT_SYS();
    END_OF_INSTR();
}
...
void x86emuOp_ret_near_IMM(u8 X86EMU_UNUSED(op1))
{
	...
	// The following detects the final return at the end of a program. -WSF
	SimControl.CheckReturn();
	// End
    START_OF_INSTR();
    DECODE_PRINTF("RET\t");
    imm = fetch_word_imm();
