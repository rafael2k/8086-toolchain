//
// disasm.c: 80x86 disassembly code
//
// This file was taken from the NASM disassembler. It allows the simulator
// to disassembler instructions when stepping and when using the 
// disassembly command. The original header is below. -WSF
//
//////////////////////////////////////////////////////////////////////
...
static char *whichcond(int condval) 
{
    ...
    return (char *)conditions[conds[condval]];
}
	     ...
	     int autosync, unsigned long prefer)
{
	    ...
	    (ins.oprs[i].segment & SEG_RMREG)) 
	{
	    ...
	    slen += sprintf(output+slen, "%lXh", ins.oprs[i].offset);	// Changed to append h, instead of 0x. -WSF
	} else if ( !(MEM_OFFS & ~(*p)->opd[i]) ) {
	    slen += sprintf(output+slen, "[%s%s%s%lXh]",				// Changed to append h, instead of 0x. -WSF
			    (segover ? segover : ""),
			    (segover ? ":" : ""),
		...
		slen += sprintf(output+slen, "%c%lXh", sign,				// Changed to append h, instead of 0x. -WSF
				ins.oprs[i].offset);
...
{
    sprintf(output, "db %02Xh", *data);		// Changed to append h, instead of 0x. -WSF
    return 1;
}
