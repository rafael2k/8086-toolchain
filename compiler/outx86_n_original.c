...
enum e_gt
{
    bytegen, wordgen, longgen, longlonggen, floatgen, nogen
};
enum e_sg
{
...
static void putamode P3 (const ADDRESS *, ap, ILEN, len, int, sa)
{
	...
	switch (len) {
	case IL1:
	    reg = REG8 (reg);	// -WSF, was: REG8LO (reg);
	    break;
	case IL2:
	    reg = REG16 (reg);
	    break;
...
PRIVATE void put_code P1 (const CODE *, ip)
{
	...
	if (ip->oper2 != NIL_ADDRESS &&
	    ip->opcode != op_line && ip->opcode != op_idiv) {
...
PRIVATE void put_storage P1 (SYM *, sp)
{
    ...
    if (is_static (sp)) {
	oprintf ("%s%u\ttimes\t%ld db 0%s", prefix, (unsigned) sp->value.l,
		 size, newline);
    } else {
	oprintf ("%s\ttimes\t%ld db 0%s", outlate (nameof (sp)), size,
		 newline);
    }
}
...
static void seg P3 (enum e_sg, segtype, const char *, segname, SIZE, al)
{
    nl ();
    if (curseg != segtype) {
	oprintf ("[segment %s]%s", segname, newline);
	curseg = segtype;
	align_type = 0L;
    }
    put_align (al);
...
{
    oprintf ("%s Generated by %s (nasm) %s from %s%s",
	     comment, PROGNAME, VERSION, in_file, newline);
    oprintf ("[group group1 bss data]%s", newline);
    oprintf ("[group group2 text]%s", newline);
    put_bseg (0L);
    put_dseg (0L);
    put_cseg (0L);
}
