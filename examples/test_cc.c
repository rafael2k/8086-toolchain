 ...
 *
 *	Originally from C86 compiler c99test.c
 *	9 Jun 2025 Greg Haerr - changed to compare long and int operations
 *
 *	This program is designed to help test new features introduced
 *	into the C86 compiler as a result of the ISO 1999 C standard
 *
...
/*
 *	Defines for using the 32 bit data type
 */
#define LONG long
 ...
 */
void	result_ll (char * prompt,  int result, int line)
{
	printf ("%4.d: %-8s %-30s $%04.4x %04.4x = %d",
			line,
	...
	{
		printf ("%4.d: FAILED - x value corrupted (now %d)\n",
				line,
	...
	{
		printf ("%4.d: FAILED - y value corrupted (now %d)\n",
				line,
...
void	result_ll_val (char * op_prompt, int val1, int val2, int result, int line)
{
	char	prompt[100];
 ...
 */
void	result_ull (char * prompt,	unsigned int result, int line)
{
	printf ("%4.d: %-8s %-30s $%04.4x %04.4x = %u",
			line,
	...
	if (ux.l[BYTE_REVERSED == FALSE] != x)
	{
		printf ("%4.d: FAILED - x value corrupted (now %u)\n",
				line,
				ux.ul[BYTE_REVERSED == FALSE]);
	}
	if (uy.l[BYTE_REVERSED == FALSE] != y)
	{
		printf ("%4.d: FAILED - y value corrupted (now %u)\n",
				line,
...
void	result_ull_val (char * op_prompt, unsigned int val1, unsigned int val2, unsigned int result, int line)
{
	char	prompt[100];

	if (0== val2)
		sprintf (prompt, "%uUL %s",
				 val1,
				 op_prompt);
	else
		sprintf (prompt, "%uUL %s %uUL",
				 val1,
				 op_prompt,
				 val2);
