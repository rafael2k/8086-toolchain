/* NOTES BY WADE FIFE:
I think the purpose of this module is to be able to add synchronization points for dissasembly.
For example, if you know in advance where data and functions are in the program, you can
add the beginning of each code section as a synchronization point. This will cause
the dissasembler to readjust the dissasembler offset when a synchronization point is reached.
This will make sure that the offset doesn't get out of alignment with the instructions.
*/
...
unsigned long next_sync(unsigned long position, unsigned long *length) 
{
    while (nsynx > 0 && synx[1].pos + synx[1].length <= position) {
		...
		while (i*2 <= nsynx) {
			j = i*2;
			if (synx[j].pos < synx[i].pos && (j+1 > nsynx || synx[j+1].pos > synx[j].pos)) {
