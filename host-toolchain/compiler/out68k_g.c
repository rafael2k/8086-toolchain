...
static void putamode P2 (const ADDRESS *, ap, ILEN, len)
{
	...
	oprintf ("a%d@(", (int) ap->preg - (int) A0);
	putconst (ap->u.offset, len);
	oprintf (",d%d:l)", (int) ap->sreg);
	break;
    case am_indx3:
	oprintf ("a%d@(", (int) ap->preg - (int) A0);
	putconst (ap->u.offset, len);
	oprintf (",a%d:l)", (int) ap->sreg - (int) A0);
	break;
    case am_indx4:
	oprintf ("a%d@(", (int) ap->preg - (int) A0);
	putconst (ap->u.offset, len);
	oprintf (",d%d:w)", (int) ap->sreg);
	break;
    case am_indxpc:
	oprintf ("pc@(");
	putconst (ap->u.offset, len);
	...
	oprintf ("pc@(");
	putconst (ap->u.offset, len);
	oprintf (",a%d:l)", (int) ap->preg - (int) A0);
	break;
    case am_rmask:
	put_rmask (ap->u.mask);
	break;
...
{
    if (mask &
	(REGMASK) ((1UL << (int) FP0) | (1UL << (int) FP1) |
		   (1UL << (int) FP2) | (1UL << (int) FP3) | (1UL << (int)
							      FP4) | (1UL <<
								      (int)
								      FP5) |
		   (1UL << (int) FP6) | (1UL << (int) FP7))) {
	oprintf ("#0x%04lx", mask >> (int) FP0);
    } else {
	oprintf ("#0x%04lx", mask);
    }
}
